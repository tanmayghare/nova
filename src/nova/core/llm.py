...
class LLM:
    ...
    def get_last_response(self) -> Optional[Any]:
        """Get the last raw response from the model provider."""
        # Check if the provider implements this optional method
        if hasattr(self._provider, 'get_last_response'):
            return self._provider.get_last_response()
        return None
    
    async def interpret_command(self, command: str) -> Dict[str, Any]:
        """Interpret a natural language command by delegating to the provider."""
        if hasattr(self._provider, 'interpret_command'):
            return await self._provider.interpret_command(command)
        else:
            # Handle cases where the underlying provider doesn't support it
            logger.error(f"The configured LLM provider ('{type(self._provider).__name__}') does not support 'interpret_command'.")
            raise NotImplementedError(f"Provider {type(self._provider).__name__} does not implement interpret_command") 